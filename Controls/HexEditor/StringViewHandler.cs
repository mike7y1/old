...
// Type: InvokedServer.Controls.HexEditor.StringViewHandler
...
namespace InvokedServer.Controls.HexEditor
{
	public class StringViewHandler
	{
		...
		public StringViewHandler(InvokedServer.Controls.HexEditor.HexEditor editor)
		{
			this._editor = editor;
		...
		public void OnKeyDown(KeyEventArgs e)
		{
			...
			else if (e.KeyCode == Keys.Up && this._editor.CaretIndex - this._editor.BytesPerLine >= 0)
			{
				...
				if (index % this._editor.BytesPerLine == 0 && this._editor.CaretPosX >= this._recStringView.X + this._recStringView.Width)
				{
		...
		{
			int num1 = (x - this._recStringView.X) / (int)this._editor.CharSize.Width;
			...
			this._editor.SetCaretStart(Math.Min(this._editor.LastVisibleByte, this._editor.FirstVisibleByte + num4 + num6 * this._editor.BytesPerLine), new Point(num4 * (int)this._editor.CharSize.Width + this._recStringView.X, num6 * this._recStringView.Height + this._recStringView.Y));
		}
		...
		public void OnMouseDragged(int x, int y)
		{
			...
			this._editor.SetCaretEnd(Math.Min(this._editor.LastVisibleByte, this._editor.FirstVisibleByte + num4 + num6 * this._editor.BytesPerLine), new Point(num4 * (int)this._editor.CharSize.Width + this._recStringView.X, num6 * this._recStringView.Height + this._recStringView.Y));
		}
		...
		public void Update(int startPositionX, Rectangle area)
		{
			...
			int width = (int)((double)charSize.Width * (double)this._editor.BytesPerLine);
			charSize = this._editor.CharSize;
			int height = (int)charSize.Height - 2;
			this._recStringView = new Rectangle(x, y, width, height);
